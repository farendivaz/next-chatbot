...
export default function ChatbotInterface({
  ...
  return (
    <div className="border-2 bg-slate-100 border-blue-600 max-w-3xl poppins rounded-xl shadow-lg w-full">
      <div className="mx-0 mb-0 w-100 rounded">
        <h4 className="bg-blue-600 mb-0 p-1 w-100 rounded-top text-center text-white font-bold text-xl rounded-t-md">
          ...
          {messageLog?.map((item, index) => {
            ...
            if (item.sender === "user") {
              return (
                <p
                  key={index}
                  className="bg-blue-600 border-2 border-white my-1 ml-auto px-3 py-2 rounded-l-3xl rounded-br-2xl text-white w-min"
                >
                  {item.message}
                </p>
              );
            ...
            // bot chat
              ...
              return (
                <div key={index}>
                  ...
                  <p className="inline-block bg-slate-100 border-2 border-blue-700 ml-1 my-1 px-2 py-1 rounded-r-3xl rounded-tl-2xl text-dark">
                    {item.message}
                  </p>
                </div>
              );
      ...
      <div className="flex my-4">
        <div className="grid gap-2 p-1 w-1/3">
          <button
            id="btn-start"
            type="submit"
            value="Submit"
